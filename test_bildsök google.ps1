$url = "https://www.google.se/search?q=pun&espv=2&biw=1462&bih=953&source=lnms&tbm=isch&sa=X&ved=0ahUKEwiwipvSvoTKAhUCCiwKHegtAxMQ_AUIBigB"
$browserAgent = 'Mozilla/5.0 (Windows NT 6.3; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/33.0.1750.146 Safari/537.36'

$web = Invoke-WebRequest -Uri $url -UserAgent $browserAgent

$result = $web.Links | 
Where-Object {$_.href -like "*imgres*"} |
ForEach-Object { ($_.href -split 'imgurl=')[-1].Split('&')[0]}

$weburl = Get-Random $result
