...
param
(
[ValidateSet("1way","2way","simultan","all")]
[string] $Option)
...
if (Test-Path -path "C:\ProgramData\iperf\iperf.exe")
{
...
switch ($Option)
{
    ...
    $result
# write-host testet startade $time1 och avslutades $time2 och tog $total_time.Seconds sekunder att genomföra, resultatet blev $result MBit/s -BackgroundColor Yellow -ForegroundColor Black
}
    '2way'{
    $time1 = Get-Date;
    ...
    $result
#write-host testet startade $time1 och avslutades $time2 och tog $total_time.Seconds sekunder att genomföra, resultat blev klient till server $result1 MBits/s och server till klient $result2 MBits/s -BackgroundColor Yellow -ForegroundColor Black
}
    'simultan' {
    $time1 = Get-Date;
   # "Testar mot servern, det tar ca 10 sekunder"
...
#write-host testet startade $time1 och avslutades $time2 och tog $total_time.Seconds sekunder att genomföra, resultat blev klient till server $result1 MBits/s och server till klient $result2 MBits/s -BackgroundColor Yellow -ForegroundColor Black
    ...
    $time2 = Get-Date;
    $total_time = $time2 - $time1;
   # write-host testet startade $time1 och avslutades $time2, resultat blev klient till server $result1 MBits/s och server till klient $result2 MBits/s och resultat i det simultana teste blev klient till server $result3 MBits/s och server till klient $result4 MBits/s -BackgroundColor Yellow -ForegroundColor Black
...
{
    Write-Host "Programmet iperf.exe finns inte katalogen C:\ProgramData\iperf\ var god lägg in det där innan du kör skriptet igen."
}
}
